...
- Accessible directement depuis la fiche conducteur
...
Score final = 
  ...
  + Disponibilité totale (0-60 points)
  + Expérience × 2 points
...
- **Utilisateur** : Consultation + attribution manuelle
...
### Entreprise de Transport (20 conducteurs, 15 tournées)
**Avant** : 3 heures par semaine pour le planning manuel
**Après** : 15 minutes avec l'IA + vérifications

### PME de Livraison (5 conducteurs, 8 tournées)
**Avant** : Gestion Excel complexe et erreurs fréquentes
**Après** : Interface simple, pas d'erreurs de permis

### Société de Messagerie (50 conducteurs, 40 tournées)
**Avant** : Impossible à gérer manuellement
**Après** : Planning complet généré en 2 minutes
...
2. **Décompresser** dans votre dossier web
3. **Créer** une base de données MySQL
4. **Copier** `config.php.example` → `config.php`
5. **Configurer** les identifiants de base de données
6. **Ouvrir** `install.php` dans le navigateur
...
- `Docs/LOGOS_TOURNEES.md` : Guide logos
...
📈 C'est un **outil évolutif** qui s'adapte à vos besoins  
